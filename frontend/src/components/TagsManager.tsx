...
export default function TagsManager({ tags, onChange, language = 'en' }: Props) {
  ...
  return (
    <div className="surface-muted flex flex-col sm:flex-row items-start sm:items-center gap-3 px-3 py-3">
      <div className="flex items-center gap-2 w-full sm:w-auto">
        ...
        />
        <button onClick={handleAdd} className="action-button w-full sm:w-auto ml-2">
          {t('addTag', language)}
        </button>
      </div>
      ...
      <div className="flex items-center gap-2 overflow-x-auto py-1">
        {tags.slice(0, 6).map((tag) => (
          <div key={tag.id} className="flex items-center gap-2 rounded px-2 py-1 border flex-shrink-0">
            ...
            <button onClick={() => handleDelete(tag.id)} className="ml-2 text-xs text-red-600">×</button>
          </div>
        ))}
      </div>
    </div>
